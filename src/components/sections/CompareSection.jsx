...
function Cell({ value }) {
  if (value === "\u2713") return <span className="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-accent-2/16 text-lg font-black text-accent-2 shadow-[0_0_24px_rgba(38,230,163,0.16)]">{value}</span>;
  if (value === "\u2717") return <span className="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-[#ff6b6b]/13 text-lg font-black text-[#ff8585]">{value}</span>;
          ...
          <table className="w-full min-w-[860px] border-separate border-spacing-0 text-[0.95rem]">
            <thead>
              <tr className="bg-[#07101d]/78">
                ...
                {columns.map((col) => (
                  <th
                    key={col.key}
                    className={`relative border-b border-white/14 p-5 text-left font-black tracking-tight ${
                      col.key === "prism" ? "text-accent-2 bg-accent-2/12 shadow-[inset_0_0_70px_rgba(38,230,163,0.13)]" : "text-[#d8e4ef]"
                    } ${
                      col.key === "sheets" ? "rounded-tr-[26px]" : ""
                    }`}
                  >
                    {col.label}
                    {col.key === "prism" && <span className="mt-2 block w-fit rounded-full bg-accent-2 px-3 py-1 text-[0.68rem] uppercase tracking-[0.12em] text-[#03111a] shadow-[0_10px_30px_rgba(38,230,163,0.22)]">Best for personal books</span>}
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.map((row, i) => (
                ...
                >
                  <td className={`border-b border-white/12 p-5 text-left font-bold text-text whitespace-nowrap ${i === rows.length - 1 ? "rounded-bl-[26px]" : ""}`}>
                    {row.label}
                  </td>
                  {columns.map((col) => (
                    <td
                      key={col.key}
                      className={`border-b border-white/12 p-5 text-left whitespace-nowrap ${
                        col.key === "prism" ? "text-text bg-accent-2/8 shadow-[inset_1px_0_0_rgba(38,230,163,0.18),inset_-1px_0_0_rgba(93,216,255,0.14)]" : "text-[#c9d6e2]"
                      } ${i === rows.length - 1 && col.key === "sheets" ? "rounded-br-[26px]" : ""}`}
                    >
                ...
                {columns.map((col) => (
                  <div
                    ...
                    className={`rounded-2xl border p-3 text-sm ${
                      col.key === "prism" ? "border-accent-2/22 bg-accent-2/9 text-text shadow-[inset_0_0_32px_rgba(38,230,163,0.08)]" : "border-white/10 bg-[#07101d]/36 text-[#c9d6e2]"
                    }`}
                  >
                    <span className={`text-[0.75rem] font-black uppercase tracking-[0.08em] block mb-1.5 ${col.key === "prism" ? "text-accent-2" : "text-[#9fb0bf]"}`}>
                      {col.label}
                    </span>
                    <Cell value={row[col.key]} />
                  </div>
