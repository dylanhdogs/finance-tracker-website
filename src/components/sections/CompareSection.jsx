    ...
    <section id="compare" className="px-5 sm:px-6 py-12 md:py-16 lg:py-[65px] mx-auto max-w-7xl scroll-mt-32">
      ...
      <Reveal direction="fade" delay={0.1}>
          ...
          <table className="w-full min-w-[860px] border-separate border-spacing-0 text-[0.95rem]">
            <thead>
              <tr className="bg-[#07101d]/78">
                ...
                {columns.map((col) => (
                  <th key={col.key} className={`relative border-b border-white/14 p-5 text-left font-black tracking-tight ${col.key === "prism" ? "text-accent-2 bg-accent-2/12 shadow-[inset_0_0_70px_rgba(38,230,163,0.13)]" : "text-[#d8e4ef]"} ${col.key === "sheets" ? "rounded-tr-[26px]" : ""}`}>
                    {col.label}
                    {col.key === "prism" && (
                      <span className="mt-2 block w-fit rounded-full bg-accent-2 px-3 py-1 text-[0.68rem] uppercase tracking-[0.12em] text-[#03111a] shadow-[0_10px_30px_rgba(38,230,163,0.22)]">Built for personal finance</span>
                    )}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {rows.map((row, i) => (
                <motion.tr key={row.label} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.04, ease: [0.16, 1, 0.3, 1] }} className={`${i % 2 === 0 ? "bg-white/[0.025]" : "bg-[#07101d]/32"} transition-colors duration-180 hover:bg-white/[0.075]`}>
                  <td className={`border-b border-white/12 p-5 text-left font-bold text-text whitespace-nowrap ${i === rows.length - 1 ? "rounded-bl-[26px]" : ""}`}>{row.label}</td>
                  {columns.map((col) => (
                    <td key={col.key} className={`border-b border-white/12 p-5 text-left whitespace-nowrap ${col.key === "prism" ? "text-text bg-accent-2/8 shadow-[inset_1px_0_0_rgba(38,230,163,0.18),inset_-1px_0_0_rgba(93,216,255,0.14)]" : "text-[#c9d6e2]"} ${i === rows.length - 1 && col.key === "sheets" ? "rounded-br-[26px]" : ""}`}>
        ...
        <div className="lg:hidden grid gap-3 mt-8">
          {rows.map((row, i) => (
            <motion.div key={row.label} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.05, ease: [0.16, 1, 0.3, 1] }} className="rounded-[24px] border border-white/12 bg-[linear-gradient(145deg,rgba(255,255,255,0.08),rgba(255,255,255,0.025))] p-4 shadow-[0_18px_70px_rgba(0,0,0,0.22)]">
              <div className="font-black text-text mb-3 text-[0.95rem]">{row.label}</div>
              <div className="grid grid-cols-2 gap-3">
                {columns.map((col) => (
                  <div key={col.key} className={`rounded-2xl border p-3 text-sm ${col.key === "prism" ? "border-accent-2/22 bg-accent-2/9 text-text shadow-[inset_0_0_32px_rgba(38,230,163,0.08)]" : "border-white/10 bg-[#07101d]/36 text-[#c9d6e2]"}`}>
